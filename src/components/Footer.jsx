...
import { betAmt, boxesSet, cashOutbetamount, changebetFix, changebetValue, fixBets, revealedFalse, setcashOutamount, togglefooter, togglemain } from '../features/mines/mineSlices'
...
const resetGame = () => {
    ...
    return (
        <footer className={`bg-blue-800  w-full md:h-20 h-2/10 rounded-2xl flex md:flex-row flex-col-reverse gap-x-2 gap-y-2 justify-center  items-center `}>
                ...
                <div className={`h-70 w-60 bg-sky-700 rounded-2xl fixed md:bottom-22 bottom-18 md:left-70  ${keyboard ? "" : "hidden"}`}>
                    <div className=' m-2 h-9/12 rounded-2xl bg-sky-700 flex flex-wrap gap-x-2 gap-y-2 justify-center items-center'>
                        {numkeys.map((num, index) => {
                            return <span key={index} onClick={() => changevalue(num)} className='w-2/7 h-1/5 rounded-xl border-black flex border justify-center items-center text-white bg-sky-700'>{(num == "delete") ? <FiDelete /> : `${num}`}</span>
                        })}
                    </div>
                ...
                <div className={`md:w-1/4 w-9/10 h-90 rounded-xl bg-slate-800 fixed md:bottom-22 bottom-18 flex flex-col ${togglefixedamt ? "" : "hidden"}`}>
                    ...
                    <div className='flex flex-wrap mx-5 h-10/12 gap-y-3 justify-between'>
                        {fixedBetArray.map((amount, index) => {
                            return <div key={index} onClick={() => changefixedamt(amount)} className={`w-4/9 rounded-xl text-white flex justify-center items-center ${(amount == betamount) ? "bg-sky-700" : "bg-sky-900"}`}>{amount.toFixed(2)}</div>
                        })}
                    </div>
                </div>
            ...
            <div className=' md:w-1/3 w-9/10 md:h-7/10 h-1/3 rounded-full flex  items-center' >
                ...
                {!disablefooter ? <div onClick={bet} className='flex  items-center justify-between w-9/10 bg-green-600 h-full rounded-2xl'><CiPlay1 className='text-white font-bold text-3xl ml-5' /><span className='text-white font-bold text-xl w-8/10  mr-10 flex justify-center'>BET</span></div>
                    : <div onClick={cashOut} className='flex  items-center justify-between w-9/10 bg-yellow-400 h-full rounded-2xl'><CiPlay1 className='text-white font-bold text-3xl ml-5' /><span className='text-white font-bold text-xl w-8/10  mr-10 flex justify-center'>Cashout{cashoutAmount.toFixed(2)}</span></div>
                }
            </div>
